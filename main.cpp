...
bool condition1(int q , int time1 , int time2){
    if(q < time1 - time2){
        return true;
    }
    return false;
}
bool condition2(int p , vector<int> periods){
    int diffrentials = 0;
    for(int i = 0 ; i < periods.size() ; i = i + 2){
        diffrentials += (periods[i + 1] - periods[i]);
    }
    if(p < diffrentials){
        return true;
    }
...
vector<bool> check_maximum_duration(vector<app> const apps){
    ...
    for(int i = 0 ; i < apps.size() ; i++){
        ...
        for(int j = 0 ; j < apps[i].periods.size() ; j = j+2){
            if(condition1(apps[i].q , apps[i].periods[j+1] , apps[i].periods[j])){
                result.push_back(false);
                break;
            }
            flag ++;
        }
        if(condition2(apps[i].p , apps[i].periods)){
            result.push_back(false);
        }
        else if(flag == apps[i].periods.size()/2){
